#!/bin/bash
#
# launch_disagg_1p2d.sh
#
# Disaggregated 1P:2D Asymmetric Multiplexing across 3 GPUs
# Supports:
#   - Single Node (a100-04): GPU 0 (P), GPU 1 (D1), GPU 2 (D2)
#   - Cross Node: Node A (GPU 0=P) -> Node B (GPU 0=D1, GPU 1=D2)
#   - Proxy: Port 8000
#
set -euo pipefail
...
LOG_PREFILL="${LOG_DIR}/disagg_1p2d_prefill_8100.log"
LOG_DEC1="${LOG_DIR}/disagg_1p2d_decode1_8201.log"
LOG_DEC2="${LOG_DIR}/disagg_1p2d_decode2_8202.log"
LOG_PROXY="${LOG_DIR}/disagg_1p2d_proxy_8000.log"

echo "================================================================="
echo " Launching Disaggregated 1P:2D (3 GPUs)"
echo " Prefill:   ${NODE_PREFILL} GPU ${GPU_P} (TP=1, Port 8100)"
echo " Decoder 1: ${NODE_DECODE}  GPU ${GPU_D1} (TP=1, Port 8201)"
...
echo "=== [2/5] Launching Prefill on GPU ${GPU_P} (Port 8100) ==="
if [ "$IS_SINGLE_NODE" -eq 1 ]; then
...
echo "=== [3/5] Launching Decoder 1 on GPU ${GPU_D1} (Port 8201) ==="
if [ "$IS_SINGLE_NODE" -eq 1 ]; then
  ...
  ssh -o BatchMode=yes -o StrictHostKeyChecking=no "${NODE_DECODE}" "nohup bash -c '
    export PATH=\"${VENV_DIR}/bin:\$PATH\"
    CUDA_VISIBLE_DEVICES=${GPU_D1} UCX_NET_DEVICES=${UCX_NET_DEV} VLLM_NIXL_SIDE_CHANNEL_HOST=\"${IP_DECODE}\" VLLM_NIXL_SIDE_CHANNEL_PORT=6001 \
...
echo "=== [4/5] Launching Decoder 2 on GPU ${GPU_D2} (Port 8202) ==="
if [ "$IS_SINGLE_NODE" -eq 1 ]; then
...
ELAPSED=0
until curl -s -f "http://${IP_DECODE}:8201/health" > /dev/null 2>&1; do
...
echo "=== [5/5] Launching Disagg 1P:2D Proxy on Port 8000 ==="
...
echo "[✓] Disagg 1P:2D Proxy is ready on Port 8000!"
echo "================================================================="
