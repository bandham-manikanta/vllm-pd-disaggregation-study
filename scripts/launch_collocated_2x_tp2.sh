#!/bin/bash
#
# launch_collocated_2x_tp2.sh
#
# Launch 2x Collocated TP=2 Engines (4 GPUs total)
# Supports:
#   - Single Node (a100-04): Engine 1 on GPUs 0,1 (:8001), Engine 2 on GPUs 2,3 (:8002)
#   - Cross Node: Engine 1 on Node A GPUs 0,1 (:8001), Engine 2 on Node B GPUs 0,1 (:8002)
...
LOG_ENG1="${LOG_DIR}/collocated_2x_tp2_engine1_8001.log"
LOG_ENG2="${LOG_DIR}/collocated_2x_tp2_engine2_8002.log"
LOG_PROXY="${LOG_DIR}/collocated_2x_tp2_proxy_8000.log"

echo "================================================================="
echo " Launching Collocated Baseline: 2x TP=2 across 4 GPUs"
echo " Engine 1: ${NODE_A} GPUs ${GPU_ENG1} (TP=2, Port 8001)"
echo " Engine 2: ${NODE_B} GPUs ${GPU_ENG2} (TP=2, Port 8002)"
...
echo "=== [2/4] Launching Collocated Engine 1 (Port 8001) ==="
if [ "$IS_SINGLE_NODE" -eq 1 ]; then
  ...
  ssh -o BatchMode=yes -o StrictHostKeyChecking=no "${NODE_A}" "nohup bash -c '
    export PATH=\"${VENV_DIR}/bin:\$PATH\"
    CUDA_VISIBLE_DEVICES=${GPU_ENG1} vllm serve \"${MODEL}\" --host 0.0.0.0 --tensor-parallel-size 2 --port 8001 --gpu-memory-utilization 0.85 >> \"${LOG_ENG1}\" 2>&1
  ' > /dev/null 2>&1 &"
  PID_ENG1=""
fi

echo "=== [3/4] Launching Collocated Engine 2 (Port 8002) ==="
if [ "$IS_SINGLE_NODE" -eq 1 ]; then
  ...
  ssh -o BatchMode=yes -o StrictHostKeyChecking=no "${NODE_B}" "nohup bash -c '
    export PATH=\"${VENV_DIR}/bin:\$PATH\"
    CUDA_VISIBLE_DEVICES=${GPU_ENG2} vllm serve \"${MODEL}\" --host 0.0.0.0 --tensor-parallel-size 2 --port 8002 --gpu-memory-utilization 0.85 >> \"${LOG_ENG2}\" 2>&1
  ' > /dev/null 2>&1 &"
  PID_ENG2=""
fi
...
echo "Waiting for Engine 1 (Port 8001)..."
ELAPSED=0
...
echo "Waiting for Engine 2 (Port 8002)..."
ELAPSED=0
...
if [ "$IS_SINGLE_NODE" -eq 1 ]; then
  ...
  ssh -o BatchMode=yes -o StrictHostKeyChecking=no "${NODE_A}" "nohup bash -c '
    export PATH=\"${VENV_DIR}/bin:\$PATH\"
    python3 \"${PROXY_SCRIPT}\" --port 8000 --backends 127.0.0.1:8001 ${IP_B}:8002 >> \"${LOG_PROXY}\" 2>&1
  ' > /dev/null 2>&1 &"
  PID_PROXY=""
fi
...
echo "[✓] Collocated 2x TP=2 Load Balancer is ready on Port 8000!"
echo "================================================================="
echo " ALL SERVICES READY! Target Endpoint: http://${IP_A}:8000/v1/completions"
echo "================================================================="
